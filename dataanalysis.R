...
#Height, is then a reasonable choice because it scales with bivalve size well across the three species.
model2 <- lm(mp_ingested_adjusted ~ height * weight * species, data = data)
vif(model2)
...
model3 <- lm(mp_ingested_adjusted ~ height * species, data = data)
vif(model3)
...
model4 <- lm(mp_ingested_adjusted ~ height * species, data = data)
model5 <- lm(mp_ingested_adjusted ~ height + species, data = data)
model6 <- lm(mp_ingested_adjusted ~ height, data = data)
model7 <- lm(mp_ingested_adjusted ~ species, data = data)

#since sample size is large (n = 48), use AIC instead of AICc
library(MuMIn)
model_sel <- model.sel(model4, model5, model6, model7, rank = 'AIC')
model_sel

#model7 has the lowest AIC but has almost identical AIC values as model4 (0.1 difference)
#but model4 models height and its interaction with species, thus is a comprehensive model
...
library(ggplot2)
myPlot <- ggplot(data, aes(height, mp_ingested_adjusted, colour = species)) +
  geom_smooth(method="lm", size = 1.2, se = FALSE) +
  geom_point(size=3) +
  theme_bw() + 
  xlab("Height (mm)") +
  ylab("Microplastic ingested (mg)") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        legend.title = element_text(face = "italic")) + 
  scale_y_continuous(limits=c(0, 60))+
  scale_color_discrete(name ="Species:", 
                       labels=c("M. meretrix", "M. lusoria")) +
